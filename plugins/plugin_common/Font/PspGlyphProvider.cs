...
namespace plugin_common.Font
{
    internal class PspGlyphProvider(FontImageData fontData, bool isFurigana) : IGlyphProvider
    {
        ...
        public CharacterInfo? GetOrDefault(ushort codePoint, Color textColor)
        {
            ...
            ColorMatrix colorMatrix = CreateColorMatrix(textColor);
            ...
            var glyphImage = fontData.Images[fontGlyph.Location.Index].Image.GetImage().Clone(context => context.Crop(srcRect).Filter(colorMatrix));
